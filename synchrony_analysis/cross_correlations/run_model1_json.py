...
def model1(file: str, tp: str, ppt: str, args):
    ...
    comb_json_path = settings.FOLDER / "json_files" / str(file + "-pd-combined_output.json")
    participant_output_path = args.OUTPUT_FOLDER / file
    os.makedirs(participant_output_path, exist_ok = True) # make participant folder in PCI dir if does not exist
    output_json_path = participant_output_path / str(file + "-segmented_analysis.json")
    output_img_path = participant_output_path / str(file + "-peak_curve.png")

    # run analysis plots and parameters
    if not os.path.isfile(output_json_path): # check if run already
        ...
        with open(participant_output_path / str(file + "-metrics.csv"), 'w') as f:
...
if __name__ == '__main__':
    ...
    args.OUTPUT_FOLDER = Path(str(settings.MODEL1_FOLDER) + "_" + args.fps + "fps") # specific to json only analysis
    os.makedirs(args.OUTPUT_FOLDER, exist_ok=True) # make cross_corr output folder if does not exist

    # XXX: uncomment for new analysis
    ...
    best_cams = pd.read_csv(str(settings.BEST_CAMERAS) + "_model1.csv")

    # run multicore
    print('Running Model 1 analysis')
    pool = mp.Pool(mp.cpu_count())
    params = [(file.Filename.replace('-pd-combined_output', ''),
                file.tp,
                file.ppt,
                args) for index, file in best_cams.iterrows()]
    ...
    for result in glob(str(args.OUTPUT_FOLDER) + "\\*\\*-metrics.csv"):
        with open(result, 'r', newline='') as csvfile:
            csv_reader = csv.reader(csvfile)
            res_rows.append(list(csv_reader)[0])

    # save as combined file
    with open(args.OUTPUT_FOLDER / "combined_results.csv", "w", newline='') as f:
        writer = csv.writer(f)
        writer.writerows(res_rows)

    end = time.time()
    print('Runtime: {}sec for {} files'.format(round(end - start), len(best_cams)))
