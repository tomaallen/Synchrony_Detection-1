...
tempDIM=size(DATA,2); % calculate dimensionality of input time-series, changed to no. of columns instead!
DIM=tempDIM;

if EMB > 1 % if EMB > 1, perform time-delayed embbedding
    for i = 1:EMB
        %size(DATA)
        %size(DATA(1+(i-1)*DEL:length(DATA)-(EMB-i)*DEL,:));
        %size(DATA(1+(i-1)*DEL:length(DATA)-(EMB-i)*DEL,:))
        
        %disp(length(DATA)-(EMB-1)*DEL)
        %disp(1+(i-1)*DEL) 
        %disp(length(DATA)-(EMB-i)*DEL)
        %disp(1+DIM*(i-1))
        %disp(DIM*i)
...
a=pdist2(DATA,DATA,"euclidean"); % create distance matrix and recurrence matirx
a=abs(a)*-1;
