...
if __name__ == "__main__":
    ...
    my_parser.add_argument('--reach_dir',
                           type=str,
                           help='reaching detection output folder with csv face',
                           default=r'Y:\Synchronised_trimmed_videos_STT\Pose_detect_output\Mom_Cam')

    # Execute the parse_args() method
    args = my_parser.parse_args()

    for ppt in os.listdir(args.reach_dir):
        if not os.path.exists(os.path.join(args.reach_dir, ppt, 'json_files', ppt + '-PD-combined_output.json')):
            print('combining json for' + ppt)
            input_folder = os.path.join(args.reach_dir, ppt)
            read_csv()
