...
def read_csv(filename=None):
    # Read the CSV file
    if filename is None:
        filename = file_path_giving_folder("csv_face")[0]
    ...
    for _id, _data in data_all.items():
        ...
        if not os.path.exists(os.path.join(reaching_const.INPUT_FOLDER, "matlab_files", str(_id))):
            os.makedirs(os.path.join(reaching_const.INPUT_FOLDER, "matlab_files", str(_id)))

        savemat(os.path.join(reaching_const.INPUT_FOLDER, "matlab_files", str(_id),
                             reaching_const.PREFIX + reaching_const.MATLAB_RAW_DATA_FILE), {'Face': _data})
    return df
...
def read_matlab_yolo(data_type=reaching_const.MATLAB_RAW_DATA_FILE, save_face=False):
    ...
    for _id in ['0', '1']:
        ...
        arr = loadmat(os.path.join(reaching_const.YOLO_INPUT_FOLDER, "matlab_files", str(_id),
                                   reaching_const.PREFIX + data_type))['Face']
        arr[:, 0] *= frame_width  # multiply the first column by 'a'
        arr[:, 1] *= frame_height  # multiply the second column by 'b
        face_data[_id] = arr
        if save_face:
            savemat(os.path.join(reaching_const.YOLO_INPUT_FOLDER, "matlab_files", str(_id),
                                 reaching_const.PREFIX + reaching_const.MATLAB_FACE_TO_OPENPOSE), {'Face': arr})
    # print(face_data)
    # input()
